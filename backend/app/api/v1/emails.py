    ...
    if data.reply_to_email_id is not None:
        reply_parent = (
            await session.execute(
                _scope_history(select(SentEmail).where(SentEmail.id == data.reply_to_email_id), user)
            )
        ).scalar_one_or_none()
...
async def get_email(
    ...
    email = (
        await session.execute(_scope_history(select(SentEmail).where(SentEmail.id == email_id), user))
    ).scalar_one_or_none()
    if email is None:
