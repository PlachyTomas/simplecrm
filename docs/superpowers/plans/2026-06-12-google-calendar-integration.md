...
- [x] Steps: TDD CRUD + visibility + validation with stubbed client; commit `feat(events): deal events CRUD with optional google sync` — 15 tests passing; fixed MissingGreenlet by passing deal_name explicitly instead of touching event.deal post-commit

**Resolved pause point (kept for history; Task 4 is done, continue with Task 5):**
